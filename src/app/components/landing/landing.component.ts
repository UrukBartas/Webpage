...
export class LandingComponent implements OnInit {
  ...
  private controlIntroductionHistory() {
    ...
    const trigger1: any = document.querySelector(
      '#introduction .trigger-animation1'
    );
    const trigger2: any = document.querySelector(
      '#introduction .trigger-animation2'
    );
    const trigger3: any = document.querySelector(
      '#introduction .trigger-animation3'
    );
    const trigger4: any = document.querySelector(
      '#introduction .trigger-animation4'
    );
    const trigger5: any = document.querySelector(
  ...
  private controlIntroductionGates() {
    ...
    const gatesInitialWidth = 100;
    const gatesFinalWidth = 160;
    const containerInitialBottom = 100;
    const containerFinalBottom = -30;
    if (visible) {
      const percentage = ((target - elementTop) / elementHeight) * 100;
      const increaseWidth =
        ((gatesFinalWidth - gatesInitialWidth) * percentage) / 100;
      const totalWidth = gatesInitialWidth + increaseWidth;

      const decreaseBottom =
        ((containerFinalBottom - containerInitialBottom) * percentage) / 100;
  ...
  private controlMythicsScroll() {
    ...
    const background: any = document.querySelector(
      '#cover .background-image-cover'
    );
